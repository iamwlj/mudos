...
void c_index() {
    ...
    case T_BUFFER:
	{
	    ...
	    i = (sp - 1)->u.number;
	    if ((i > (LPC_INT)sp->u.buf->size) || (i < 0))
...
void c_rindex() {
    ...
    case T_BUFFER:
	{
	    ...
	    i = sp->u.buf->size - (unsigned int)(sp - 1)->u.number;
...
void c_add_eq P1(int, is_void) {
	...
	if (sp->type == T_NUMBER) {
	    ...
	    lval->u.number += (LPC_INT)sp->u.real;
	    /* both sides are numbers, no freeing required */
	} else {
	...
	else *global_lvalue_byte.u.lvalue_byte += (unsigned char)sp->u.number;
	break;
    default:
...
static int compare_switch_entries P2(string_switch_entry_t *, p1,
    ...
    return (int)(((POINTER_INT)p1->string - (POINTER_INT)p2->string));
}
...
void fix_switches P1(string_switch_entry_t **, tables) {
	...
	quickSort((char *)(*tables), (int)(p - *tables),
