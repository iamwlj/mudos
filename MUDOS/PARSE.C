...
static void push_parse_globals() {
    ...
    sp->u.error_handler = (void(*)(void))parse_clean_up;
	 ...
	 int, 		num_arg)
{
    ...
    parse_warr = explode_string(cmd, (int)strlen(cmd), " ", 1);
    
    /* Array of pattern elements */
    parse_patarr = explode_string(pattern, (int)strlen(pattern), " ", 1);
...
prepos_parse P4(array_t *, warr, int *, cix_in, int *, fail, svalue_t *, prepos)
{
	...
	if (!strchr(tmp, ' ')) {
	    ...
	    tarr = explode_string(tmp, (int)strlen(tmp), " ", 1);
...
find_string P3(char *, str, array_t *, warr, int *, cix_in)
{
	...
	split = explode_string(str, (int)strlen(str), " ", 1);
     ...
     parse_to_plural P1(char *, str)
{
    ...
    words = explode_string(str, (int)strlen(str), " ", 1);
