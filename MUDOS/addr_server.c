...
void init_conn_sock P2(int, port_num, char *, ipaddress)
{
     ...
     */
    if ((conn_fd = (int)socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
...
void new_conn_handler()
{
    ...
    new_fd = (int)accept(conn_fd, (struct sockaddr *) & client, (int *) &client_len);
    if (new_fd == -1) {
	...
	OS_socket_write(new_fd, message, (int)strlen(message));
...
/* Failed :( */
	sprintf(out_buf, "%s 0\n", &buf[sizeof(int)]);
	DBG(("%s", out_buf));
	OS_socket_write(all_conns[conn_index].fd, out_buf, (int)strlen(out_buf));
	return 0;
    } else {
/* Success! */
...
int addr_main P2(int, argc, char **, argv)
{
