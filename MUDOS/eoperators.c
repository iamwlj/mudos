...
f_div_eq()
{
	...
	case T_NUMBER|T_REAL:
	{
		...
		sp->u.real = (float)(argp->u.number /= (int)sp->u.real);
	    }
	    break;
	}
...
f_range P1(int, code)
{
    ...
    switch(sp->type) {
        ...
        case T_BUFFER:
        {
            ...
            if (to >= len) to = len - 1;
            {
                buffer_t *nbuf = allocate_buffer((int)(to - from + 1));
...
f_sub_eq()
{
	...
	case T_NUMBER|T_REAL:
	{
		...
		sp->type = T_REAL;
		sp->u.real = argp->u.real -= sp->u.number;
	    } else sp->u.real = (float)(argp->u.number -= (int)sp->u.real);
	    break;
	}
...
#define L_ADDR	(SWITCH_CASE_SIZE + (int)sizeof(char *))
...
f_switch()
{
	...
	if (i == 14) {
	    ...
	    s -= d;
	    if (s >= 0 && s < (int)((zz-l)/sizeof(short))) {
