...
static void numadd (outbuffer_t *, LPC_INT num);
...
static void numadd P2(outbuffer_t *, outbuf, LPC_INT, num)
{
    LPC_INT i, num_l,               /* length of num as a string */
...
void svalue_to_string P5(svalue_t *, obj, outbuffer_t *, outbuf, int, indent, int, trailing, int, indent2)
{
	...
	break;
    case T_REAL:
	outbuf_addv(outbuf, DBL_PRINT_FMT, obj->u.real);
	break;
    case T_STRING:
	outbuf_add(outbuf, "\"");
...
static int add_table P1(cst **, table)
{
	...
	end = (int)(tab_d[i + 1].start - tab_di - 1);
    
	for (done = 0; done != end && tab_di[done] != '\n'; done++)
	    ;
	add_justified(tab_di, (done > (int)tab->size ? tab->size : done),
			...
			if (finfo & INFO_COLS) {
			    ...
			    *temp = ALLOCATE(cst, TAG_TEMPORARY, "string_print: 3");
			    ...
			    tmp = ((format_str[fpos] != '\n') 
				   && (format_str[fpos] != '\0'))
				|| ((finfo & INFO_ARRAY)
				    && ((int)nelemno < (argv + sprintf_state->cur_arg)->u.arr->size));
...
#define TABLE carg->u.string
			...
			add_justified(carg->u.string, slen, &pad, fs, finfo,
				      (((format_str[fpos] != '\n') && (format_str[fpos] != '\0'))
				       || ((finfo & INFO_ARRAY) && ((int)nelemno < (argv + sprintf_state->cur_arg)->u.arr->size)))
		    ...
		    *cheat = '%';
			...
			cheat[i++] = '.';
			if (pres >= sizeof(temp))
			    sprintf(cheat + i, "%d", (int)(sizeof(temp) - 1));
			else
			    sprintf(cheat + i, "%d", pres);
			i += (unsigned int)strlen(cheat + i);
		    }
		    switch (finfo & INFO_T) {
		    case INFO_T_INT:
                        cheat[i++] = 'l';
		...
		if (!(finfo & INFO_ARRAY))
		    break;
		if ((int)nelemno >= (argv + sprintf_state->cur_arg)->u.arr->size)
		    break;
