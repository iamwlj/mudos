...
#define CURRENT_BLOCK_SIZE (int)(prog_code - mem_block[current_block].block)
...
c_generate_node P1(parse_node_t *, expr) {
    ...
    case NODE_STRING:
	ins_vstring("C_STRING("INT_PRINT_FMT");\n", expr->v.number);
	break;
    case NODE_REAL:
	ins_vstring("push_real("DBL_PRINT_FMT");\n", expr->v.real);
	break;
    case NODE_NUMBER:
	ins_vstring("push_number("INT_PRINT_FMT");\n", expr->v.number);
	break;
    case NODE_LAND_LOR:
	c_generate_node(expr->l.expr);
	c_generate_forward_branch((char)expr->v.number);
	c_generate_node(expr->r.expr);
	if (expr->l.expr->kind == NODE_BRANCH_LINK) {
	    c_update_forward_branch_links((char)expr->v.number,expr->l.expr);
	}
    ...
    case NODE_SWITCH_DIRECT:
	{
		...
		position = (int)(prog_code - mem_block[current_block].block);
...
c_update_branch_list P1(parse_node_t *, bl) {
    if (bl) {
	do {
	    upd_jump((int)bl->l.number, label);
	} while ((bl = bl->v.expr));
	add_label();
    }
}
