...
disassemble P5(FILE *, f, char *, code, int, start, int, end, program_t *, prog)
{
    ...
    LPC_INT i, j, instr, iarg, is_efun, ri;
    unsigned short sarg;
	...
	for (i = 0; i < (int) NUM_FUNS_D; i++) {
		...
		offsets[i << 1] = (short)prog->function_table[i].address;
	    }
	    offsets[(i << 1) + 1] = (short)i;
	}
#ifdef _SEQUENT_
...
#endif
	    COPY_SHORT(&sarg, pc);
	    offset = (unsigned short)((pc - code) + (unsigned short) sarg);
	...
	case F_BBRANCH_LT:
	    COPY_SHORT(&sarg, pc);
	    offset = (unsigned short)((pc - code) - sarg);
	    sprintf(buff, "%04x (%04x)", (unsigned) sarg, (unsigned) offset);
	    pc += 2;
	    break;
	...
	case F_GLOBAL:
		...
		sprintf(buff, "%s", variable_name(prog, (int)iarg));
	    else
		sprintf(buff, "<out of range %d>", (int)iarg);
	    pc++;
	    break;
	...
	case F_LOOP_COND_LOCAL:
	    ...
	    sprintf(buff, "LV%d < LV%d bbranch_when_non_zero %04x (%04x)",
		    (int)i, (int)iarg, (int)sarg, offset);
	    break;
	case F_STRING:
	    COPY_SHORT(&sarg, pc);
	    if (sarg < NUM_STRS)
	...
	case F_REAL:
		...
		COPY_FLOAT(&farg, pc);
		sprintf(buff, DBL_PRINT_FMT, farg);
		pc += 4;
		break;
	    }
	...
	case F_SWITCH:
		...
		disassemble(f, code, (int)(pc - code + 7), addr + stable, prog);
	...
	case F_EFUN3:
		...
		sprintf(buff, "%d", (int)(instr - F_EFUN0));
	    }
	    break;
	case 0:
	...
	fprintf(f, "%s %s\n", query_instr_name((int)instr), buff);
    }
