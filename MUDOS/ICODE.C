...
static void ins_real PROT((LPC_FLOAT));
...
static void write_number PROT((LPC_INT));
static void ins_int PROT((LPC_INT));
#if SIZEOF_PTR == 8
...
static int try_to_push PROT((int, LPC_INT));
...
static void ins_real P1(LPC_FLOAT, l)
{
    LPC_FLOAT f = l;
 ...
 * that correct byte order is used, regardless of machine architecture.
 */
static void ins_int P1(LPC_INT, l)
{
 ...
 */
static void write_small_number P1(LPC_INT, val) {
    if (try_to_push(PUSH_NUMBER, val)) return;
    ins_byte(F_BYTE);
    ins_byte((unsigned char)val);
}

static void write_number P1(LPC_INT, val)
{
    if ((val & ~0xff) == 0)
...
switch_to_line P1(int, line) {
    LPC_INT sz = CURRENT_PROGRAM_SIZE - last_size_generated;
    ADDRESS_TYPE s;
    unsigned char *p;
...
static int
try_to_push P2(int, kind, LPC_INT, value) {
    if (push_state) {
	if (value <= PUSH_MASK) {
	    if (push_state == 1)
		initialize_push();
	    push_state++;
	    ins_byte((unsigned char)(kind | value));
	    if (push_state == 255)
		end_pushes();
	    return 1;
	} else end_pushes();
...
i_generate_node P1(parse_node_t *, expr) {
    ...
    case NODE_FUNCTION:
	  ...
	  num = (unsigned short)(FUNCTION_TEMP(expr->v.number)->u.index);
	  FUNC(num)->address = generate_function(FUNC(num),
				  expr->r.expr, (int)expr->l.number);
	  break;
      }
    ...
    case NODE_OPCODE:
	end_pushes();
	ins_byte((unsigned char)expr->v.number);
	break;
	...
	end_pushes();
	ins_byte((unsigned char)expr->v.number);
	ins_byte((unsigned char)expr->type);
	break;
	...
	end_pushes();
	ins_byte((unsigned char)expr->v.number);
	ins_byte((unsigned char)expr->l.number);
	break;
    case NODE_OPCODE_2:
	end_pushes();
	ins_byte((unsigned char)expr->v.number);
	...
	if (expr->v.number == F_LOOP_COND_NUMBER)
	    ...
	    ins_byte((unsigned char)expr->r.number);
	break;
    case NODE_RETURN:
	{
	    int n;
    ...
    case NODE_LAND_LOR:
	i_generate_node(expr->l.expr);
	i_generate_forward_branch((char)expr->v.number);
	i_generate_node(expr->r.expr);
	if (expr->l.expr->kind == NODE_BRANCH_LINK)
	    i_update_forward_branch_links((char)expr->v.number,expr->l.expr);
	else 
	...
	end_pushes();
	ins_byte((unsigned char)expr->v.number);
	ins_byte((unsigned char)(expr->l.number >> 16));
    ...
    case NODE_PARAMETER:
	{
	    LPC_INT which = expr->v.number + current_num_values;
	    if (try_to_push(PUSH_LOCAL, which)) break;
	    ins_byte(F_LOCAL);
	    ins_byte((unsigned char)which);
	    break;
	}
    case NODE_PARAMETER_LVALUE:
	end_pushes();
	ins_byte(F_LOCAL_LVALUE);
	ins_byte((unsigned char)expr->v.number + current_num_values);
	break;
    case NODE_IF:
    ...
    case NODE_FOREACH:
	{
	    ...
	    ins_byte(tmp);
	    ins_byte((unsigned char)expr->l.expr->l.number);
	    if (expr->r.expr)
		ins_byte((unsigned char)expr->r.expr->l.number);
	}
	break;
    ...
    case NODE_SWITCH_RANGES:
	{
		...
		upd_short((int)(addr + 5), (int)(expr->v.expr->v.number - addr),
			  "switch");
		expr->v.expr = expr->v.expr->l.expr;
	    } else {
		upd_short((int)(addr + 5), (int)(CURRENT_PROGRAM_SIZE - addr), "switch");
	    }
	    ...
	    ins_short(0);
	    /* build table */
	    upd_short((int)(addr + 1), (int)(CURRENT_PROGRAM_SIZE - addr), "switch");
#ifdef BINARIES
		...
		while (pn) {
		    ins_rel_short((int)(pn->v.number - addr));
		    pn = pn->l.expr;
		}
	    ...
	    upd_short((int)last_break, (int)(CURRENT_PROGRAM_SIZE - last_break), "switch break");
	    upd_short((int)(addr+3), (int)(CURRENT_PROGRAM_SIZE - addr), "switch");
	    break;
	}
    case NODE_CATCH:
	{
	    ...
	    upd_short((int)addr, (int)(CURRENT_PROGRAM_SIZE - addr), "catch");
	    break;
	}
	...
	case FP_FUNCTIONAL | FP_NOT_BINDABLE:
	    {
		LPC_INT addr, save_current_num_values = current_num_values;
		ins_byte((unsigned char)(expr->v.number >> 8));
		addr = CURRENT_PROGRAM_SIZE;
		ins_short(0);
		current_num_values = expr->r.expr ? expr->r.expr->kind : 0;
		i_generate_node(expr->l.expr);
		current_num_values = (int)save_current_num_values;
		end_pushes();
		ins_byte(F_RETURN);
		upd_short((int)addr, (int)(CURRENT_PROGRAM_SIZE - addr - 2),
			  "function pointer");
		break;
	    }
	}
    ...
    case NODE_ANON_FUNC:
	{
	    ...
	    ins_byte((unsigned char)(expr->v.number & 0xff));
	    ins_byte((unsigned char)expr->l.number);
	    addr = CURRENT_PROGRAM_SIZE;
	    ins_short(0);
	    i_generate_node(expr->r.expr);
    ...
    case NODE_EFUN:
	{
		...
		if (expr->l.number < 4 && instrs[f].max_arg != -1)
		    ins_byte((unsigned char)(F_EFUN0 + expr->l.number));
		else {
		    ins_byte(F_EFUNV);
		    ins_byte((unsigned char)expr->l.number);
		}
...
static void i_generate_loop P4(int, test_first, parse_node_t *, block,
    ...
    } else i_branch_backwards(generate_conditional_branch(test), (int)pos);
...
void i_generate_forward_branch P1(char, b) {
    ...
    forward_branches[nforward_branches++] = (int)CURRENT_PROGRAM_SIZE;
    ins_short(0);
}
...
i_update_forward_branch P1(char *, what) {
    ...
    upd_short(forward_branches[nforward_branches],
        (int)(CURRENT_PROGRAM_SIZE - forward_branches[nforward_branches]),
	      what);
}
...
void i_update_forward_branch_links P2(char, kind, parse_node_t *, link_start) {
    ...
    upd_short(forward_branches[nforward_branches], 
        (int)(CURRENT_PROGRAM_SIZE - forward_branches[nforward_branches]),
	      "&& or ||");
    do {
	i = (int)link_start->v.number;
	upd_byte(i-1, kind);
	upd_short(i, (int)(CURRENT_PROGRAM_SIZE - i), "&& or ||");
...
i_update_branch_list P2(parse_node_t *, bl, char *, what) {
    LPC_INT current_size;
    
    end_pushes();
    current_size = CURRENT_PROGRAM_SIZE;

    while (bl) {
	upd_short((int)bl->l.number, (int)(current_size - bl->l.number), what);
	bl = bl->v.expr;
    }
}
...
i_generate_else() {
    ...
    end_pushes();
    ins_byte(F_BRANCH);
    forward_branches[nforward_branches-1] = (int)CURRENT_PROGRAM_SIZE;
    ins_short(0);

    /* update the old branch to point to this point */
    upd_short(old, (int)(CURRENT_PROGRAM_SIZE - old), "if");
}
