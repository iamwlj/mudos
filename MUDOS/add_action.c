...
static void notify_no_command PROT((void))
{
	...
	if (p.s) {
	    tell_object(command_giver, p.s, (int)strlen(p.s));
	    free_string(p.s);
	    command_giver->interactive->default_err_message.s = 0;
	} else {
	    tell_object(command_giver, default_fail_message, (int)strlen(default_fail_message));
	}
    }
}
...
static int user_parser P1(char *, buff)
{
	...
	*p = ' ';
	...
	if (s->flags & V_NOSPACE) {
	    int l1 = (int)strlen(s->verb);
	    int l2 = (int)strlen(verb_buff);
...
void f_command PROT((void))
{
    ...
    {
	char buff[1000];
	LPC_INT save_eval_cost = eval_cost;
