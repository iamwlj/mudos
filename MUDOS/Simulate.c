...
{
    static int i;
    char *p = DXALLOC((int)strlen(str) + 10, TAG_OBJ_NAME, "make_new_name");
...
void destruct_object P1(object_t *, ob)
{
	...
	sp->u.error_handler = (void(*)(void))fix_object_names;
...
static void send_say P3(object_t *, ob, char *, text, array_t *, avoid)
{
    ...
    tell_object(ob, text, (int)strlen(text));
}
...
void tell_room P3(object_t *, room, svalue_t *, v, array_t *, avoid)
{
    ...
    case T_NUMBER:
	buff = txt_buf;
	sprintf(buff, INT_PRINT_FMT, v->u.number);
	break;
    case T_REAL:
	buff = txt_buf;
	sprintf(buff, DBL_PRINT_FMT, v->u.real);
	break;
    default:
	...
	if (!ob->interactive) {
	    ...
	    tell_object(ob, buff, (int)strlen(buff));
	    if (ob->flags & O_DESTRUCTED)
		break;
	}
    }
...
void shout_string P1(char *, str)
{
	...
	if (!(ob->flags & O_LISTENER) || (ob == command_giver)
	    ...
	    continue;
	tell_object(ob, str, (int)strlen(str));
    }
}
...
void print_svalue P1(svalue_t *, arg)
{
	...
	case T_OBJECT:
	    ...
	    tell_object(command_giver, tbuf, (int)strlen(tbuf));
	    break;
	case T_NUMBER:
	    sprintf(tbuf, INT_PRINT_FMT, arg->u.number);
	    tell_object(command_giver, tbuf, (int)strlen(tbuf));
	    break;
	case T_REAL:
	    sprintf(tbuf, DBL_PRINT_FMT, arg->u.real);
	    tell_object(command_giver, tbuf, (int)strlen(tbuf));
	    break;
	case T_ARRAY:
	    tell_object(command_giver, "<ARRAY>", (int)strlen("<ARRAY>"));
	    break;
	case T_MAPPING:
	    tell_object(command_giver, "<MAPPING>", (int)strlen("<MAPPING>"));
	    break;
	case T_FUNCTION:
	    tell_object(command_giver, "<FUNCTION>", (int)strlen("<FUNCTION>"));
	    break;
#ifndef NO_BUFFER_TYPE
	case T_BUFFER:
	    tell_object(command_giver, "<BUFFER>", (int)strlen("<BUFFER>"));
	    break;
#endif
	default:
	    tell_object(command_giver, "<UNKNOWN>", (int)strlen("<UNKNOWN>"));
	    break;
	}
    return;
}
