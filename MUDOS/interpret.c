...
INLINE_STATIC void push_lvalue_range P1(int, code)
{
    ...
    ind2 = (int)((code & 0x01) ? (size - sp->u.number) : sp->u.number);
    if (++ind2 < 0 || (ind2 > size))
    ...
    ind1 = (int)((code & 0x10) ? (size - sp->u.number) : sp->u.number);
...
#  endif
	...
	case F_ADD:
		...
		case T_NUMBER:
			...
			case T_STRING:
				...
				sprintf(buff, INT_PRINT_FMT, (sp+1)->u.number);
		...
		case T_REAL:
			...
			case T_STRING:
				...
				sprintf(buff, DBL_PRINT_FMT, (sp+1)->u.real);
				EXTEND_SVALUE_STRING(sp, buff, "f_add: 2");
				break;
			    }
			default:
	...
	case F_ADD_EQ:
		...
		if (sp->type == T_STRING) {
		    ...
		    sprintf(buff, INT_PRINT_FMT, sp->u.number);
		    EXTEND_SVALUE_STRING(lval, buff, "f_add_eq: 2");
		} else if (sp->type == T_REAL) {
		    char buff[40];
		     
		    sprintf(buff, DBL_PRINT_FMT, sp->u.real);
		    EXTEND_SVALUE_STRING(lval, buff, "f_add_eq: 2");
		} else {
	    ...
	    case T_LVALUE_BYTE:
		{
		    ...
		    c = (char)(*global_lvalue_byte.u.lvalue_byte + sp->u.number);
	...
	case F_FOREACH:
		...
		if (sp->type == T_STRING) {
		    ...
		    sp->subtype = (short)SVALUE_STRLEN(sp - 1);
		} else {
	...
	case F_PRE_INC:
	    ...
	    case T_REAL:
		sp->type = T_REAL;
		sp->u.real = (LPC_FLOAT)(++lval->u.number);
		break;
	    case T_LVALUE_BYTE:
		if (global_lvalue_byte.subtype == 0 &&
		    *global_lvalue_byte.u.lvalue_byte == (unsigned char)255)
	...
	case F_END_TIME_EXPRESSION:
		...
		get_usec_clock(&sec, &usec);
		usec = (long)(sec - (sp - 1)->u.number) * 1000000 + (long)(usec - sp->u.number);
		sp -= 2;
		IF_DEBUG(stack_in_use_as_temporary--);
		push_number(usec);
		break;
...
int apply_low P3(char *, fun, object_t *, ob, int, num_arg)
{
  ...
  retry_for_shadow:
    ...
    ix = (LPC_INT)((pfun >> 2)^(pfun >> (2 + APPLY_CACHE_BITS))^(pprog >> 2)^(pprog >> (2 + APPLY_CACHE_BITS)));
	...
	if (entry->progp) {
	    ...
	    int index = (int)(funp - entry->progp->function_table);
			...
			char **, ret_file, int *, ret_line )
{
    ...
    *ret_line = 0;
    ...
    translate_absolute_line((int)abs_line, &progp->file_info[2],
			    &file_idx, ret_line);
...
array_t *get_svalue_trace()
{
    ...
    v = allocate_empty_array((int)(csp - &control_stack[0]) + 1);
    for (p = &control_stack[0]; p < csp; p++) {
...
#ifdef _WIN64
#define strtolpcint(val, result, base) _strtoi64(val, result, base)
#else
#define strtolpcint(val, result, base) strtol(val, result, base)
#endif
...
int inter_sscanf P4(svalue_t *, arg, svalue_t *, s0, svalue_t *, s1, int, num_arg)
{
	...
	case 'd':
	    {
		tmp = in_string;
		num = (LPC_INT) strtolpcint(in_string, &in_string, base);
	...
	while (*in_string) {
		...
		if (!skipme) {
		    ...
		    newmatch = new_string(skipme = (int)(in_string - match), "inter_sscanf");
...
void reset_machine P1(int, first)
{
	...
	pop_n_elems((int)(sp - start_of_stack + 1));
...
void restore_context P1(error_context_t *, econ) {
    ...
    pop_n_elems((int)(sp - econ->save_sp));
