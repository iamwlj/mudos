...
alloc_new_string P2(char *, string, int, h)
{
    ...
    b = (block_t *) DXALLOC((int)size, TAG_SHARED_STRING, "alloc_new_string");
    strncpy(STRING(b), string, len);
    STRING(b)[len] = '\0';	/* strncpy doesn't put on \0 if 'from' too
				 * long */
    SIZE(b) = (unsigned int)(len > UINT_MAX ? UINT_MAX : len);
    REFS(b) = 1;
    NEXT(b) = base_table[h];
    base_table[h] = b;
...
#endif
{
    ...
    ret = (char *)DXALLOC((int)strlen(str) + 1, TAG_STRING, tag);
    strcpy(ret, str);
    return ret;
}
