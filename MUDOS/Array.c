...
INLINE_STATIC long alist_cmp P2(svalue_t *, p1, svalue_t *, p2)
{
    ...
    if ((d = (long)(p1->u.number - p2->u.number))){
    	if(d == LONG_MIN)
...
array_t *subtract_array P2(array_t *, minuend, array_t *, subtrahend) {
    ...
    free_array(minuend);
    msize = (long)(dest - difference->item);
    if (!msize) {
