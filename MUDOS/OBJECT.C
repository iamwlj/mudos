...
INLINE int svalue_save_size P1(svalue_t *, v)
{
	...
	{
	    char buf[256];
	    sprintf(buf, DBL_PRINT_FMT, v->u.real);
...
restore_interior_string P2(char **, val, svalue_t *, sv)
{
	...
	case '\\':
		...
		if ((*new++ = *cp++)) {
		    ...
		    *new = '\0';
		    *val = cp;
		    sv->u.string = new_string(len = (int)(new - start),
					      "restore_string");
...
static int parse_numeric P3(char **, cpp, unsigned char, c, svalue_t *, dest) 
{
	...
	*cpp = cp;
	...
	if ((c = *cp++) == '+') {
	    ...
	    f1 = (LPC_FLOAT)(res * pow(10.0, expo));
	} else if (c == '-') {
	    while ((c = *cp++) && isdigit(c)) {
		expo *= 10;
		expo += (c - '0');
...
restore_mapping P2(char **,str, svalue_t *, sv)
{
	...
	*str += 2;
	...
	oi = (unsigned long)MAP_POINTER_HASH(key.u.number);
	i = oi & mask;
	if ((elt2 = elt = a[i])) {
	    do {
...
save_object P3(object_t *, ob, char *, file, int, save_zeros)
{
    ...
    if (sel == -1) sel = (int)strlen(SAVE_EXTENSION);
