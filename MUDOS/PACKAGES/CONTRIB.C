...
f_terminal_colour PROT((void))
{
    ...
    if (st_num_arg >= 3) {
	if (st_num_arg == 4)
	    indent = (int)(sp--)->u.number;
	wrap = (int)(sp--)->u.number;
	...
	if ((cp = findstring(parts[i]))) {
	    POINTER_INT tmp = MAP_POINTER_HASH(cp);
