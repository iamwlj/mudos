...
f_socket_bind PROT((void))
{
	...
	i = socket_bind(fd, (int)arg[1].u.number, (num_arg == 3 ? arg[2].u.string : 0));
	pop_n_elems(num_arg - 1);
        sp->u.number = i;
    } else {
	pop_n_elems(num_arg - 1);
...
f_socket_accept PROT((void))
{
    ...
    get_socket_address(fd = (int)(sp-2)->u.number, addr, &port, 0);
...
f_socket_release PROT((void))
{
    ...
    (sp-2)->u.number = VALID_SOCKET("release") ?
      socket_release((int)(sp - 2)->u.number, (sp - 1)->u.ob, sp) :
	EESECURITY;
...
f_socket_acquire PROT((void))
{
    ...
    (sp-3)->u.number = VALID_SOCKET("acquire") ?
      socket_acquire((int)(sp - 3)->u.number, (sp - 2),
		     (sp - 1), sp) : EESECURITY;
...
/* This is so we can get the address of interactives as well. */
    ...
    get_socket_address((int)sp->u.number, addr, &port, local);
...
f_socket_status PROT((void))
{
     ...
     if (st_num_arg) {
	 info = socket_status((int)sp->u.number);
