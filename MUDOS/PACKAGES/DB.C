...
#ifdef USE_SQLITE
#ifdef _WIN64
#pragma comment(lib, ".\\SQLite\\lib\\X64\\sqlite3.lib")
#else
#pragma comment(lib, ".\\SQLite\\lib\\X86\\sqlite3.lib")
#endif
...
static db_defn_t SQLite3 = {
    ...
    NULL,      SQLite3_cleanup, NULL,          SQLite3_errormsg, NULL};
...
void f_db_close PROT((void))
{
    ...
    db = find_db_conn((int)sp->u.number);
    if (!db) {
...
void f_db_connect PROT((void))
{
    ...
    switch (st_num_arg) {
	case 4: type     = (int)(sp - (args++))->u.number;
...
void f_db_exec PROT((void))
{
    ...
    db = find_db_conn((int)(sp-1)->u.number);
    if (!db) {
...
void f_db_fetch PROT((void))
{
    ...
    if (db->type->fetch) {
	ret = db->type->fetch(&(db->c), (int)sp->u.number);
    } else {
	ret = &the_null_array;
    }
...
static int SQLite3_connect(dbconn_t *c, const char *host, const char *database,
  ...
  sprintf(filename, "%s.db", database);
...
#ifdef _WIN64
#pragma comment(lib, ".\\mysql\\50\\lib\\X64\\mysqlclient.lib")
#pragma comment(linker, "/nodefaultlib:libcmt.lib")
#else
#pragma comment(lib, ".\\mysql\\50\\lib\\X86\\mysqlclient.lib")
#endif
#elif 80 == MUDOS_MYSQL_VERSION
#ifdef _WIN64
#pragma comment(lib, ".\\mysql\\80\\lib\\X64\\mysqlclient.lib")
#pragma comment(linker, "/nodefaultlib:libcmt.lib")
#else
#pragma comment(lib, ".\\mysql\\80\\lib\\X86\\mysqlcppconn-static.lib")
#pragma comment(linker, "/nodefaultlib:libcmt.lib")
#endif
#endif
...
void f_db_fetch PROT((void))
{
    ...
    db = find_db_conn((int)(int)(sp-1)->u.number);
    if (!db) {
	error("Attempt to fetch from an invalid database handle\n");
    }
...
{
	return (int)mysql_affected_rows(c->mysql.handle);
}
...
static int MySQL_execute P2(dbconn_t *, c, char *, s)
{
	...
	if (c->mysql.results) {
	    return (int)mysql_num_rows(c->mysql.results);
	}
...
static array_t *MySQL_fetch P2(dbconn_t *, c, int, row)
{
	...
	if (!field) {
		...
		case FIELD_TYPE_DOUBLE:
		    v->item[i].type = T_REAL;
		    v->item[i].u.real = (float)atof(target_row[i]);
		    break;
...
void __cdecl _dosmaperr(unsigned long e) {
