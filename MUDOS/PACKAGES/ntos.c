...
static const char *code[]  = {"$NOR$","$BLK$","$RED$","$GRN$","$YEL$","$BLU$","$MAG$","$CYN$","$WHT$",
"$HIR$","$HIG$","$HIY$","$HIB$","$HIM$","$HIC$","$HIW$",
"$HBBLK$","$HBRED$","$HBGRN$","$HBYEL$","$HBBLU$","$HBMAG$","$HBCYN$","$HBWHT$",
"$BBLK$","$BRED$","$BGRN$","$BYEL$","$BBLU$","$BMAG$","$BCYN$","$BWHT$",
"$BOLD$","$HIREV$"};

static const char *color[] = {"[m","[30m","[31m","[32m","[33m","[34m","[35m","[36m","[1;30m",
...
static void ntos_itoa (svalue_t * sp) {
	...
	if( sp->type == T_REAL )
	{
        ...
        if( sp->type == T_NUMBER )
        {
        	...
        	new_str = new_string(i+negative, "ntos_itoa");
...
/* ³ı·¨´¦Àíº¯Ê½ */
/* n1 : ´óÕıÕûÊı(±»³ıÊı), n2 : Ğ¡ÕıÕûÊı(³ıÊı) */
static char *division (char * n1, char * n2) {
	...
	char *ret, *tmp, *str;
   	...
   	if( !n2[0] || n2[0] == 48 ) error("(division) ³ıÊıÎª 0");
