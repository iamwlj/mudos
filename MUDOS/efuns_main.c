...
{
    if (st_num_arg == 2) {
	(sp-1)->u.arr = allocate_array2((int)(sp-1)->u.number, sp);
	pop_stack();
    } else {
	sp->u.arr = allocate_array((int)sp->u.number);
    }
    sp->type = T_ARRAY;
}
#endif
...
f_allocate_buffer PROT((void))
{
    ...
    buf = allocate_buffer((int)sp->u.number);
    if (buf) {
	pop_stack();
...
f_allocate_mapping PROT((void))
{
	...
	sp->u.map = allocate_mapping((int)sp->u.number);
    } else {
	error("Bad argument 1 to allocate_mapping()\n");
    }
    sp->type = T_MAPPING;
...
f_call_out PROT((void))
{
	...
	ret = (int)new_call_out(current_object, arg, (int)arg[1].u.number, num, arg + 2);
...
{
    int i, n = (int)(csp - &control_stack[0] + 1);
    array_t *ret;
...
f_ed PROT((void))
{
	...
	pop_stack();
        ...
        if (sp->type == T_NUMBER) {
            ed_start((sp - 2)->u.string, 0, (sp - 1)->u.string, (int)sp->u.number,
                     current_object);
...
f_member_array PROT((void))
{
    ...
    if (sp->type == T_STRING) {
        ...
        if (i > (int)SVALUE_STRLEN(sp)) error("Index to start search from in member_array() is > string length.\n");
        if ((res = strchr(sp->u.string + i, (int)(sp-1)->u.number)))
...
f_random PROT((void))
{
    ...
    sp->u.number = random_number((long)sp->u.number);
}
#endif
...
f_read_bytes PROT((void))
{
    ...
    arg = sp - num_arg + 1;
    if (num_arg > 1)
        start = (int)arg[1].u.number;
    if (num_arg == 3) {
        len = (int)arg[2].u.number;
    }
...
f_replace_string PROT((void))
{
    ...
    if (rlen <= plen) {
        ...
        if (plen > 1) { /* pattern length > 1, jump table most efficient */
            ...
            dst2 += (slimit - src);
	    *dst2 = 0;
	    arg->u.string = extend_string(dst1, (int)(dst2 - dst1));
        ...
        *dst2 = '\0';
         ...
         */
        push_malloced_string(extend_string(dst1, (int)(dst2 - dst1)));
    }
}
#endif
...
f_set_eval_limit PROT((void))
{
    switch (sp->u.number) {
    case 0:
	sp->u.number = (int)(eval_cost = max_cost);
	break;
    case -1:
	sp->u.number = (int)eval_cost;
	break;
    case 1:
	sp->u.number = (int)max_cost;
	break;
    default:
...
{
    set_heart_beat(current_object, (int)(sp--)->u.number);
}
#endif
...
f_stat PROT((void))
{
    ...
    if (stat(path, &buf) != -1) {
        if (buf.st_mode & S_IFREG) {    /* if a regular file */
            ...
            v->item[1].u.number = (int)buf.st_mtime;
...
f__to_float PROT((void))
{
    ...
    switch(sp->type) {
        case T_NUMBER:
            sp->type = T_REAL;
            sp->u.real = (float) sp->u.number;
            break;
        case T_STRING:
            sscanf(sp->u.string, DBL_PRINT_FMT, &temp);
            free_string_svalue(sp);
            sp->type = T_REAL;
            sp->u.real = (float)temp;
    }
}
#endif
...
f_uptime PROT((void))
{
    push_number((LPC_INT)(current_time - boot_time));
}
#endif
...
f_write_bytes PROT((void))
{
    ...
    switch(sp->type) {
        case T_NUMBER:
        {
            ...
            netint = htonl((int)sp->u.number);       /* convert to network
                                                 * byte-order */
            netbuf = (char *) &netint;
            i = write_bytes((sp - 2)->u.string, (int)(sp - 1)->u.number, netbuf,
                            sizeof(int));
            break;
	}
        ...
        case T_BUFFER:
        {
            i = write_bytes((sp - 2)->u.string, (int)(sp - 1)->u.number,
    ...
    switch(sp->type) {
        case T_NUMBER:
        {
            ...
            netint = htonl((int)sp->u.number);       /* convert to network
                                                 * byte-order */
            netbuf = (char *) &netint;
            i = write_buffer((sp - 2)->u.buf, (int)(sp - 1)->u.number, netbuf,
                            sizeof(int));
            break;
	}
        ...
        case T_BUFFER:
        {
            i = write_buffer((sp - 2)->u.buf, (int)(sp - 1)->u.number,
                            (char *) sp->u.buf->item, sp->u.buf->size);
            break;
	}
...
{
    if (st_num_arg == 2) {
        (sp - 1)->u.ob->next_reset = current_time + (int)sp->u.number;
