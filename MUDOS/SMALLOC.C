...
#ifdef _WIN64
#define SIZE_INDEX(u_array, size) 	(*(u*) ((char*)u_array+(-8+size)*2))
#define SIZE_PNT_INDEX(u_array, size)	(*(u**)((char*)u_array+(-8+size)*2))
#else
#define SIZE_INDEX(u_array, size) 	(*(u*) ((char*)u_array-8+size))
#define SIZE_PNT_INDEX(u_array, size)	(*(u**)((char*)u_array-8+size))
#endif
    /* i = (size - 8) >> 2; */
    count_up(small_alloc_stat, (unsigned int)size);
    ...
    if (temp = SIZE_PNT_INDEX(sfltable, size)) {	/* allocate from the
							 * free list */
	count_back(small_free_stat, (unsigned int)size);
	temp++;
	...
	*next_unused = (u)(LPC_INT)last_small_chunk;
    ...
    *s_size_ptr(next_unused) = (u)(size >> 2);
    unused_size -= (u)size;
...
static void add_to_free_list P1(u *, ptr)
{
    ...
    printf("size:%d\n", size);
#endif
    q = (free_block_t *)(LPC_INT)size;	/* this assignment is a hint for
					 * register choice */
