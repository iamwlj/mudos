...
void clean_up_locals()
{
    ...
    offset = (int)(locals_ptr + current_number_of_locals - locals);
    while (offset--) {
...
void reallocate_locals() {
    int offset;
    offset = (int)(type_of_locals_ptr - type_of_locals);
...
static int add_new_function_entry() {
    LPC_INT index = mem_block[A_FUNCTION_DEFS].current_size / sizeof(compiler_temp_t);
...
static void copy_new_function P5(program_t *, prog, int, index,
    ...
    FUNCTION_TEMP(where)->offset = (unsigned short)(NUM_INHERITS - 1);
...
static int find_class_member P3(int, which, char *, name, unsigned char *, type) {
	...
	if (type) *type = (unsigned char)cme[i].type;
	return i;
    }
}

int lookup_any_class_member P2(char *, name, unsigned char *, type) {
    LPC_INT nc = mem_block[A_CLASS_DEF].current_size / sizeof(class_def_t);
    int i, ret = -1, nret;
...
void copy_structures P1(program_t *, prog) {
    ...
    ident_hash_elem_t *ihe;
    char *str;
    LPC_INT sm_off = mem_block[A_CLASS_MEMBER].current_size / sizeof(class_member_entry_t);
    LPC_INT sd_off = mem_block[A_CLASS_DEF].current_size / sizeof(class_def_t);
    int i, j, offset;
	...
	if (ihe->dn.class_num == -1) {
	    ihe->dn.class_num = (signed short)(sd_off++);
	    ihe->sem_value++;
	} else {
	...
	sd->index = (unsigned short)sm_off;
	sm_off += sd->size;
...
static void overload_function P6(program_t *, prog, int, index,
	...
	*newdef = *FUNCTION_TEMP(oldindex);
    ...
    newdef->offset = (unsigned short)(NUM_INHERITS - 1);
...
{
    inherit_t *ip;
    LPC_INT num_inherits, super_length;
	...
	super_length = 0;
        ...
        super_name = name;
        real_name = p+2;
        super_length = (int)(real_name - super_name - 2);
    }
	...
	for (; num_inherits > 0; ip++, num_inherits--) {
		...
		ret = (int)(node->l.number + ip->function_index_offset);
                           ...
                           int, flags, int, type)
{
	...
	*((unsigned short *)mem_block[A_ARGUMENT_INDEX].block + num) = 
        (unsigned short)(mem_block[A_ARGUMENT_TYPES].current_size / sizeof(unsigned short));
...
#define STRING_HASH(var,str) \
    var = (long)((LPC_INT)str ^ (LPC_INT)str >> 16); \
    var = (var ^ var >> 8) & 0xff;
...
short store_prog_string P1(char *, str)
{
	...
	*tagp |= mask;
	...
	i = (short)(mem_block[A_STRINGS].current_size / sizeof str - 1);
    }
    PROG_STRING(i) = str;
...
int validate_function_call P2(int, f, parse_node_t *, args)
{
	...
	if ((prog = FUNCTION_PROG(f))) {
	    int which = (int)(FUNCTION_TEMP(f)->u.func - prog->function_table);
	    if (prog->type_start) {
	    ...
	    LPC_INT which = FUNCTION_TEMP(f)->u.index;
...
promote_to_float P1(parse_node_t *, node) {
	...
	node->kind = NODE_REAL;
	node->v.real = (LPC_FLOAT)node->v.number;
	return node;
    }
    expr = new_node();
    expr->kind = NODE_EFUN;
...
promote_to_int P1(parse_node_t *, node) {
	...
	node->v.number = (LPC_INT)node->v.real;
	return node;
    }
    expr = new_node();
    expr->kind = NODE_EFUN;
...
validate_efun_call P2(int, f, parse_node_t *, args) {
	...
	args->type = (unsigned short)predefs[f].ret_type;
...
static void handle_functions() {
    LPC_INT num_func, total_func;
    LPC_INT i;
    compiler_temp_t *cur_def;
    LPC_INT new_index, num_def;
    LPC_INT final_index;
    inherit_t *inheritp;
	...
	comp_sorted_funcs = CALLOCATE(num_func, unsigned short,
 	...
 	i = num_func;
	while (i--) func_index_map[i] = (unsigned short)i;
	
	quickSort(func_index_map, (int)num_func, sizeof(unsigned short), 
		  compare_funcs);
	
	i = num_func;
	while (i--)
	    comp_sorted_funcs[func_index_map[i]] = (unsigned short)i;
	...
	for (i = 0; i < num_def; i++) {
	    ...
	    comp_def_index_map[i] = (unsigned short)final_index;
		...
		if (new_index != final_index) {
		    prog_flags[new_index] = (unsigned short)(FUNC_ALIAS | final_index);
		} 
	    }
	}
    }
...
static program_t *epilog PROT((void)) {
    ...
    size = mudos_align(sizeof (program_t));
	...
	if (i != A_LINENUMBERS && i != A_FILE_INFO && 
	    ...
	    i != A_ARGUMENT_INDEX)
	    size += mudos_align(mem_block[i].current_size);
    
    num_func = (int)(mem_block[A_FUNCTIONS].current_size/sizeof(function_t));

    while (num_func && 
	   FUNC(func_index_map[num_func - 1])->address == ADDRESS_MAX) 
	num_func--;

    size += mudos_align((num_func * sizeof(function_t)));   /* A_FUNCTIONS */

    /* function flags */
    size += mudos_align(((comp_last_inherited + num_func) * sizeof(unsigned short)));

    /* A_ARGUMENT_INDEX */
    if (mem_block[A_ARGUMENT_INDEX].current_size)
	size += mudos_align(num_func * sizeof(unsigned short)); 
    ...
    *prog = NULL_program;
    ...
    p += mudos_align(sizeof(program_t));
    prog->program = p;
    ...
    p += mudos_align(sizeof(function_t) * num_func);
    ...
    p += mudos_align(sizeof(unsigned short) * (comp_last_inherited + num_func));
	...
	while (i--)
	    dest[i] = *((unsigned short *) mem_block[A_ARGUMENT_INDEX].block
			+ func_index_map[i]);
	p += mudos_align(num_func * sizeof(unsigned short));
    } else {
    ...
    prog->num_classes = (unsigned short)(mem_block[A_CLASS_DEF].current_size /
	  sizeof (class_def_t));
    ...
    prog->strings = (char **)p;
    prog->num_strings = (unsigned short)(mem_block[A_STRINGS].current_size /
	  sizeof (char *));
    copy_in(A_STRINGS, &p);

    prog->num_variables_defined = (unsigned short)(mem_block[A_VAR_NAME].current_size /
	sizeof (char *));
    prog->num_variables_total = (unsigned short)(mem_block[A_VAR_TEMP].current_size /
	  sizeof (variable_t));
    ...
    prog->num_inherited = (unsigned short)(mem_block[A_INHERITS].current_size /
	  sizeof (inherit_t));
    ...
    for (i = 0; i < mem_block[A_FUNCTION_DEFS].current_size / (int)sizeof(*fundefp); i++) {
...
static void clean_parser() {
     ...
     * Free function stuff.
     */
    for (i = 0; i < mem_block[A_FUNCTIONS].current_size / (int)sizeof(function_t); i++) {
	funp = FUNC(i);
	if (funp->name) free_string(funp->name);
    }

    for (i = 0; i < mem_block[A_FUNCTION_DEFS].current_size / 
	     (int)sizeof(compiler_temp_t); i++) {
	fundefp = FUNCTION_TEMP(i)->next;
	while (fundefp) {
	    nextdefp = fundefp->next;
	    FREE((char *) fundefp);
...
void prepare_cases P2(parse_node_t *, pn, int, start) {
	...
	quickSort((char *)ce_start, (int)(ce_end - ce_start), (int)sizeof(parse_node_t *),
		  string_case_compare);
    else
	quickSort((char *)ce_start, (int)(ce_end - ce_start), (int)sizeof(parse_node_t *),
		  case_compare);
...
add_program_file P2(char *, name, int, top) {
    if (!top)
	add_to_mem_block(A_INCLUDES, name, (int)strlen(name)+1);
